...
def main():
    ...
    # Create a text input for user commands
    user_input = st.text_input("Type or say your command here")
...
def execute_command(user_input):
    if user_input.lower().startswith("alfred"):
        # Text-to-speech
        speak(user_input[3:])
    else:
        # Speech recognition
        recognized_text = takeCommandMic()
        st.write(f"Voice Assistant (Heard): {recognized_text}")
...
# st.title("Alfred AI Assistant Assistant")
...
#         # Text-to-speech
#         speak(user_input[3:])
#         st.success(f'Voice Assistant (Said): {user_input[3:]}')
#     else:
#         # Speech recognition
#         recognized_text = new_func()
